...
def run(data_dir, working_dir, epochs, batch_sizes):
    ...
    for i, (model_factory, train_val_tests_percentage, audio_seconds, (data_ops_name, data_ops_factory), patience) in enumerate(product(hyperparams['model_factories'], hyperparams['train_val_test_percentages'], hyperparams['seconds'], hyperparams['data_operations_factories'], hyperparams['patiences'])):
        ...
        for j, batch_size in enumerate(batch_sizes):
            logging.info(
                f"------------- Trying batch size {j + 1} / {len(batch_sizes)} ({round((j + 1) / float(len(batch_sizes)) * 100, 2)}%) ------------")
            logging.info(
                f'Model: {model_factory.get_model_name()} , seconds: {audio_seconds} , batch_size: {batch_size} , early_stopping_patience: {patience} data_ops: {data_ops_name}')
            ...
            cbs = get_callbacks(checkpoints_dir, logdir,
                                last_epoch_file_path, patience)

            try:
                model_runner.run(model, train_ds, val_ds, cbs=cbs,
